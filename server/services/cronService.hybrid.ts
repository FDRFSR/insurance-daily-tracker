// cronService.ts - Template scheduling service using require/module.exports pattern
...
interface CronJob {
  id: number;
  task: any; // ScheduledTask from node-cron
...
module.exports = {
  ...
  scheduleTemplate: (templateId: number, config: RecurrenceConfig) => 
    cronServiceInstance.scheduleTemplate(templateId, config),
  unscheduleTemplate: (templateId: number) => 
    cronServiceInstance.unscheduleTemplate(templateId)
};

// Also try ES module export for compatibility
export const cronService = cronServiceInstance;
export const scheduleTemplate = (templateId: number, config: RecurrenceConfig) => 
  cronServiceInstance.scheduleTemplate(templateId, config);
export const unscheduleTemplate = (templateId: number) => 
  cronServiceInstance.unscheduleTemplate(templateId);
