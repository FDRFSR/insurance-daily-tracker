...
import { googleCalendarDb, GoogleCalendarConfig, TaskCalendarMapping } from './googleCalendarDatabase';
...
export class CalendarSyncService {
  ...
  async performSync(options: SyncOptions = { 
    ...
    try {
      ...
      // Log inizio sync
      await googleCalendarDb.logSyncOperation({
        operation: 'sync',
        entityType: 'task',
        entityId: 'all',
        details: JSON.stringify({ options, phase: 'start' }),
        success: true
      });
      ...
      await googleCalendarDb.logSyncOperation({
        ...
        entityType: 'task',
        entityId: 'all',
        details: JSON.stringify({ ...result, phase: 'complete' }),
      ...
      await googleCalendarDb.logSyncOperation({
        ...
        entityType: 'task',
        entityId: 'all',
        details: JSON.stringify({ error: errorMessage, phase: 'error' }),
        success: false,
        errorMessage
      });
  ...
  private async syncFromGoogleCalendar(
    ...
    try {
      ...
      const timeMin = options.dateRange?.start || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString(); // 30 giorni fa
      const timeMax = options.dateRange?.end || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(); // 30 giorni avanti
      let events = await googleCalendarService.getEvents(timeMin, timeMax, calendarId);

      // Filtra eventi creati da InsuraTask per evitare duplicati
      events = events.filter((event: any) => !this.isInsuraTaskEvent(event));
      ...
      for (const event of events) {
        try {
          ...
          const existingMapping = googleCalendarDb.getTaskCalendarMappingByEventId(event.id!);
          
          if (existingMapping) {
            // Aggiorna task esistente se evento è più recente
            await this.updateTaskFromEvent(event as any, existingMapping, result, options);
          } else {
            // Crea nuovo task
            await this.createTaskFromEvent(event as any, calendarId, result, options);
          }
  ...
  private async syncToGoogleCalendar(
    ...
    try {
      ...
      for (const task of tasks) {
        try {
          // Controlla se esiste già mapping
          const existingMapping = googleCalendarDb.getTaskCalendarMappingByTaskId(task.id);
  ...
  private async createTaskFromEvent(
    ...
    if (!options.dryRun) {
      ...
      // Crea mapping
      await googleCalendarDb.createTaskCalendarMapping({
        taskId: newTask.id,
  ...
  private async updateTaskFromEvent(
    ...
    if (!task) {
      ...
      await googleCalendarDb.deleteTaskCalendarMapping(mapping.taskId);
      return;
    }
    ...
    if (!options.dryRun) {
      ...
      await googleCalendarDb.updateTaskCalendarMappingStatus(mapping.taskId, 'synced');
    }
  ...
  private async createEventFromTask(
    ...
    if (!options.dryRun) {
      const newEvent = await googleCalendarService.createEvent(eventData, calendarId);
  ...
  private async updateEventFromTask(
    ...
    try {
      ...
      const existingEvent = await googleCalendarService.getEvent(mapping.googleEventId, mapping.calendarId);
      if (!existingEvent) {
      ...
      if (!options.dryRun) {
        await googleCalendarService.updateEvent(mapping.googleEventId, eventUpdates, mapping.calendarId);
  ...
  private async detectAndResolveConflicts(result: SyncResult, options: SyncOptions): Promise<void> {
    ...
    for (const mapping of conflictedMappings) {
      try {
        ...
        try {
          event = await googleCalendarService.getEvent(mapping.googleEventId, mapping.calendarId) as any;
        } catch {
          event = null;
        }
  ...
  async syncSingleTask(taskId: number): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const calendarId = config.calendarId || 'primary';
      const existingMapping = googleCalendarDb.getTaskCalendarMappingByTaskId(taskId);

      if (existingMapping) {
        // Aggiorna evento esistente
        const eventUpdates = this.convertTaskToEventData(task);
        await googleCalendarService.updateEvent(existingMapping.googleEventId, eventUpdates, existingMapping.calendarId);
        await googleCalendarDb.updateTaskCalendarMappingStatus(taskId, 'synced');
      } else {
        // Crea nuovo evento
        if (!task.dueDate) {
  ...
  async unsyncTask(taskId: number): Promise<{ success: boolean; error?: string }> {
    try {
      const mapping = googleCalendarDb.getTaskCalendarMappingByTaskId(taskId);
      if (!mapping) {
      ...
      // Rimuovi mapping
      await googleCalendarDb.deleteTaskCalendarMapping(taskId);
  ...
  getSyncStatus(): {
    ...
    stats: ReturnType<typeof googleCalendarDb.getSyncStats>;
  } {
    const config = googleCalendarDb.getGoogleCalendarConfig();
